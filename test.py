...
message = "Загрузка '{0}' файла завершена успешно ".format(file_name);
    ...
    try:
        ...
        for level in file.columns:
            ...
            for indx in range(len(file.index)):
                ...
                #command = "INSERT INTO data.region_period_indicators(region_id, indicator_id, period_id, value) VALUES ({0}, {1}, {2}, {3}, {4})".format(xregion_id, xindicator_id, xperiod_id, xvalue, xfile_id);
                #cursor.execute(command)
                #connection.commit()
